GAL Core version 3.0.2 is now available from:
...
GAL Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zGAL spends, automint calculation adjustments, and other various updates/fixes.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zGAL mint operation (zGAL spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzgal` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzgal=0` at the command-line when starting the client, or add `backupzgal=0` to their `galerians.conf` file.
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the GAL address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of zGal serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzgal startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zGal spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zGal balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zGal control quantity/amount fixes (rejectedpromise)
