...
#include "ui_zgalcontroldialog.h"
...
std::set<std::string> ZGalControlDialog::setSelectedMints;
std::set<CMintMeta> ZGalControlDialog::setMints;

ZGalControlDialog::ZGalControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGalControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGalControlDialog::~ZGalControlDialog()
{
    delete ui;
}

void ZGalControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGalControlDialog::updateList()
{
...
void ZGalControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGalControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGal_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGalControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGalControlDialog::GetSelectedMints()
{
...
void ZGalControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
