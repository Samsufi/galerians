...
void PrivacyDialog::on_pushButtonZGalControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZGalControlDialog* zGalControl = new ZGalControlDialog(this);
    zGalControl->setModel(walletModel);
    zGalControl->exec();
}

void PrivacyDialog::setZGalControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGalSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGAL()
{
    ...
    if (!ZGalControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZGalControlDialog::GetSelectedMints();
    ...
    // Clear zgal selector in case it was used
    ZGalControlDialog::setSelectedMints.clear();
    ui->labelzGalSelected_int->setText(QString("0"));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zgalTracker->GetMints(false);
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzGAL->setToolTip(tr("PrivacyDialog", "Enter an amount of PIV to convert to zGAL", 0));
