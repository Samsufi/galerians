...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGaleriansAmount"))
        settings.setValue("nAnonymizeGaleriansAmount", 1000);

    nAnonymizeGaleriansAmount = settings.value("nAnonymizeGaleriansAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGaleriansAmount"))
        SoftSetArg("-anonymizegaleriansamount", settings.value("nAnonymizeGaleriansAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in galerians.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGaleriansAmount:
            return QVariant(nAnonymizeGaleriansAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGaleriansAmount:
            nAnonymizeGaleriansAmount = value.toInt();
            settings.setValue("nAnonymizeGaleriansAmount", nAnonymizeGaleriansAmount);
            emit anonymizeGaleriansAmountChanged(nAnonymizeGaleriansAmount);
            break;
        case CoinControlFeatures:
