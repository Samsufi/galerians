...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GAL address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no GAL: URI
...
{
    // Convert galerians:// to galerians:
    //
    //    Cannot handle this later, because galerians:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open galerians.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GAL.lnk";
}
...
{
    return GetAutostartDir() / "galerians.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a galerians.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the galerians app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add galerians app to startup item list
