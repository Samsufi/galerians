...
CzGALWallet::CzGALWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgal seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGALSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGALSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zgal seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgal wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
void CzGALWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgalTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGALWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    // Add to zgalTracker which also adds to database
    pwalletMain->zgalTracker->Add(dMint, true);
