        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zgals that have been in mempool long
                    //and higher priority to zgals that are large in value
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GALMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zgalTracker->RemovePending(pblock->vtx[1].GetHash());
        return error("GALMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("GALMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("galerians-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running GALMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
