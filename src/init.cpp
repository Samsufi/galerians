...
#include "config/galerians-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("galerians-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "galerians.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "galeriansd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the PIV and zGAL money supply statistics") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, galerians, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GAL specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-zgalstake=<n>", strprintf(_("Enable or disable staking functionality for zGAL inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzgal=<n>", strprintf(_("Enable automatic wallet backups triggered after each zGAL minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgalbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zGAL backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizegaleriansamount=<n>     " + strprintf(_("Keep N PIV anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The GAL Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("galerians-loadblk");
...
/** Sanity checks
 *  Ensure that GAL is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GAL Core is shutting down."));
...
#endif
    // Make sure only a single GAL process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GAL Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //GAL: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //GAL specific: zerocoin and spork DB's
                ...
                // GAL: load previous sessions sporks if we have them.
                ...
                // GAL: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GAL Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart GAL Core to complete") << "\n";
        ...
        bool fEnableZGalBackups = GetBoolArg("-backupzgal", true);
        pwalletMain->setZGalAutoBackups(fEnableZGalBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zgalTracker->Init();
...
//    }
//
//    nAnonymizeGaleriansAmount = GetArg("-anonymizegaleriansamount", 0);
//    if (nAnonymizeGaleriansAmount > 999999) nAnonymizeGaleriansAmount = 999999;
//    if (nAnonymizeGaleriansAmount < 2) nAnonymizeGaleriansAmount = 2;
    ...
    LogPrintf("Anonymize GAL Amount %d\n", nAnonymizeGaleriansAmount);
